...
public abstract class PlatformSupport {
  ...
  // TODO should consider spinning this into its own class, rather than platform support getting
  // ever bigger
  ...
  protected static Pattern regularsplitter =
      Pattern.compile(
          "(?<=\n"
              + ")|(?=\n"
              + ")|(?<=_)|(?=_)|(?<=\\\\)|(?=\\\\)|(?<=[^\\x00-\\xff])|(?=[^\\x00-\\xff])|"
              + "(?<=\\p{InHiragana})|(?=\\p{InHiragana})|(?<=\\p{InKatakana})|(?=\\p{InKatakana})|"
  ...
  protected static Pattern regularsplitterMultiline =
      Pattern.compile(
          "(?<= )|(?= )|(?<=\n"
              + ")|(?=\n"
              + ")|(?<=_)|(?=_)|(?<=\\\\)|(?=\\\\)|(?<=[^\\x00-\\xff])|(?=[^\\x00-\\xff])|"
  ...
  public BitmapFont getFont(
      int size, String text, boolean flipped, boolean border, boolean fallback) {
    FreeTypeFontGenerator generator =
        fallback ? fallbackFontGenerator : getGeneratorForString(text);
