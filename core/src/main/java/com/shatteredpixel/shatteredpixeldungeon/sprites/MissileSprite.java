...
public class MissileSprite extends ItemSprite implements Tweener.Listener {
	...
	static {
		...
		ANGULAR_SPEEDS.put(QliphothLasher.Forest_Kill.class, 0);
		...
		ANGULAR_SPEEDS.put(MagicFlyBlade.class, 0);

		ANGULAR_SPEEDS.put(StreamerKnife.class,       0);

		ANGULAR_SPEEDS.put(RedBlock.class,       0);

		ANGULAR_SPEEDS.put(RedBloodMoon.RedBloodMoonEX.class,       0);
	...
	private void setup( PointF from, PointF to, Item item, Callback listener ){
		...
		if (item instanceof Dart
			...
			speed *= 1.1f;
				...
				&& (Dungeon.hero.belongings.weapon() instanceof DiedCrossBow
				|| Dungeon.hero.belongings.secondWep() instanceof DiedCrossBow)){
			speed *= 3f;
		} else if (item instanceof Dart
			...
			speed *= 1.5f;
		}else if (item instanceof SniperSupport.FrostSnipeArrow
