...
public class WndCatalog extends WndTabbed {
        ...
        private void updateList() {
            ...
            Collections.sort(itemClasses, new Comparator<Class<? extends Item>>() {
                ...
                public int compare(Class<? extends Item> a, Class<? extends Item> b) {
                    ...
                    if (known.get(a) && Catalog.isSeen(a))
                        result -= 2;
                    if (known.get(b) && Catalog.isSeen(b))
                        result += 2;
                    if (Catalog.isSeen(a))
                        result--;
                    if (Catalog.isSeen(b))
            ...
            for (Class<? extends Item> itemClass : itemClasses) {
                ...
                boolean itemIDed = true;
                boolean itemSeen = true;
                ...
                ScrollingListPane.ListItem listItem = new ScrollingListPane.ListItem(
                        (itemIDed && itemSeen) ? new ItemSprite(item)
                                : new ItemSprite(ItemSpriteSheet.SOMETHING + spriteIndexes[currentItemIdx]),
                        null,
