...
public class TrapPlacer extends TestItem{
    ...
    private class SettingsWindow extends Window {
        ...
        public SettingsWindow(){
            ...
            Set<Integer> keys = trapLib.keySet();
            double maxTrapCount = Collections.max( keys ) + 1;
            int ROWS = (int) Math.ceil( maxTrapCount / COLS );
            boolean shouldContinue = true;

            for( int i = 0; i < ROWS && shouldContinue; ++i ){
                for( int j = 0;j < COLS && shouldContinue; ++j ){
                    final int index = i*COLS + j;
                    if( index >= ( maxTrapCount - 1 ) )
            ...
            cb.setRect(1, top + ROWS * BTN_SIZE + 4, WIDTH / 2f, 16 );
            add( cb );

            selected.setPos(WIDTH / 2f + 6, top + ROWS * BTN_SIZE + 4 + ( cb.height() - 7f ) / 2f );
