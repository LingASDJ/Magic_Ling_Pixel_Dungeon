...
public class vM0_6_7_X_Changes {
  ...
  public static void add_v0_6_20_Changes(ArrayList<ChangeInfo> changeInfos) {
    ...
    changes2.addButton(
        new ChangeButton(Icons.get(Icons.INFO), "圣诞与常规两个主界面Logo", "现在圣诞和常规有两个主界面logo，未来还会有更多。"));
    ...
    changes3.addButton(
        new ChangeButton(
            ...
            Messages.get(ChangesScene.class, "bugfixes", new Object[0]),
            Messages.get(vM0_6_7_X_Changes.class, "bug_06X40", new Object[0])));
    ...
    changes4.addButton(
        new ChangeButton(new ItemSprite(ItemSpriteSheet.DG20), "教程重制计划", "教程文档正在返修，敬请期待"));
    ChangeInfo changes5 = new ChangeInfo("调整", false, (String) null);
    changes5.hardlight(8421504);
    changeInfos.add(changes5);
    changes5.addButton(
        new ChangeButton(
            new ItemSprite(ItemSpriteSheet.REDDRAGON), "核子可乐", "核子可乐现在不能在红龙之王处被获得，但会在未来的娱乐模式中加入。"));
    changes5.addButton(
        new ChangeButton(
            new ItemSprite(ItemSpriteSheet.DG26),
            "玫瑰秘卷",
            "玫瑰秘卷无敌时间从35回合改为5回合，且不能在此期间使用吸血圣杯。制造配方能量消耗从20改为14"));
    changes5.addButton(
        new ChangeButton(
            Icons.get(Icons.CHALLENGE_OFF), "挑战重制计划", "药水癔症，基因突变，暴力Boss将会在未来重新加入，现在暂时禁用！"));
    changes5.addButton(
        new ChangeButton(new ItemSprite(ItemSpriteSheet.DG24), "极度秘卷", "暂时移除，将在未来加入。"));
    changes5.addButton(
        new ChangeButton(
  ...
  public static void add_v0_6_19_Changes(ArrayList<ChangeInfo> changeInfos) {
    ...
    changesx.addButton(
        new ChangeButton(
            new ItemSprite(ItemSpriteSheet.AMULET), ("0层改动"), ("现在0层不可以随进随出了，且水晶之心的背景故事略做调整")));
