...
public class HellBurning extends Buff implements Hero.Doom {
    ...
    public boolean act() {
        if (target.isAlive() && !target.isImmune(getClass())) {
            ...
            if (target instanceof Boss) {
                ...
                target.damage(damage, this, Char.DamageType.MAGIC);
            } else if (target instanceof Mob) {
                Mob mob = (Mob) target;
                boolean hasEliteOrMutant = mob.buffs(ChampionEnemy.class) != null;
                damage = (int) (mob.HT * NORMAL_MOB_DAMAGE_PERCENT);
                if (hasEliteOrMutant) {
                    target.damage(damage, this, Char.DamageType.MAGIC);
                } else {
                    target.damage(damage, this, Char.DamageType.REAL);
                }
            } else if (target instanceof Hero) {
                Hero hero = (Hero) target;
                damage = Random.NormalIntRange(2, 4 + Dungeon.depth / 5 + hero.lvl / 5);
        ...
        if (Dungeon.level.flamable[target.pos] && Blob.volumeAt(target.pos, HellFire.class) == 0) {
            GameScene.add(Blob.seed(target.pos, 7, HellFire.class));
        }
    ...
    @Override
    public int icon() {
        return BuffIndicator.HELLBURING;
    }

    @Override
    public void fx(boolean on) {
        if (on) target.sprite.add(CharSprite.State.HELLBURING);
        else target.sprite.remove(CharSprite.State.HELLBURING);
    }
