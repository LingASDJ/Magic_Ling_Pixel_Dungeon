...
public class DimandKing extends Boss {
  ...
  public void die(Object cause) {
    ...
    for (Mob mob : (Iterable<Mob>) Dungeon.level.mobs.clone()) {
      if (mob instanceof DKMonk || mob instanceof DKGhoul || mob instanceof DKWarlock) {
        mob.die(cause);
      }
    }
  ...
  public static class DeathRattleSummon extends Buff {
    ...
    public Mob idToMob() {
      switch (summonCate) {
        case 0:
          return Reflection.newInstance(DKGhoul.class);
        case 1:
          return Reflection.newInstance(DKWarlock.class);
        case 2:
          return Reflection.newInstance(DKMonk.class);
        case 3:
  ...
  public void doYogLasers() {
    ...
    if (!enemy.rooted) {
      for (int i : targetedCells) {
        ...
        sprite.parent.add(
            new Beam.DeathRayS(
                DungeonTilemap.raisedTileCenterToWorld(i),
    ...
    if (abilityCooldown <= 0 && HP < HT * 0.8f) {
      ...
      abilityCooldown +=
          Random.NormalFloat(
              MIN_ABILITY_CD - 2 * (1 - (HP * 1f / HT)), MAX_ABILITY_CD - 5 * (1 - (HP * 1f / HT)));
    } else {
      spend(TICK);
    }
