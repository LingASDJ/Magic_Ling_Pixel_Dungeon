...
public abstract class Mob extends Char {
  ...
  protected Char chooseEnemy() {
    ...
    // if we are an alert enemy, auto-hunt a target that is affected by aggression, even another
    // enemy
  ...
  protected boolean getCloser(int target) {
    ...
    if (Dungeon.level.adjacent(pos, target)) {
      ...
      if (path == null
          ...
          || path.size() > 2 * Dungeon.level.distance(pos, target)) newPath = true;
      ...
      if (!newPath) {
        ...
        if (!Dungeon.level.passable[nextCell]
          ...
          newPath = true;
          // If the next cell on the path can't be moved into, see if there is another cell that
          // could replace it
          if (!path.isEmpty()) {
              ...
              if (Dungeon.level.adjacent(pos, nextCell + i)
                ...
                if (Dungeon.level.passable[nextCell + i]
                    ...
                    && (!Char.hasProp(this, Char.Property.LARGE)
                        || Dungeon.level.openSpace[nextCell + i])
      ...
      if (newPath) {
        ...
        PathFinder.Path full =
            Dungeon.findPath(this, target, Dungeon.level.passable, fieldOfView, true);
        if (state != HUNTING) {
          path = full;
        } else {
          // otherwise, check if other characters are forcing us to take a very slow route
          // and don't try to go around them yet in response, basically assume their blockage is
          // temporary
          PathFinder.Path ignoreChars =
              Dungeon.findPath(this, target, Dungeon.level.passable, fieldOfView, false);
          if (ignoreChars != null && (full == null || full.size() > 2 * ignoreChars.size())) {
            ...
            if (!Dungeon.level.passable[ignoreChars.getFirst()]
                ...
                || (Char.hasProp(this, Char.Property.LARGE)
                    && !Dungeon.level.openSpace[ignoreChars.getFirst()])
  ...
  public float attackDelay() {
    ...
    return super.attackDelay()
        * (Dungeon.isChallenged(Challenges.SBSG) ? (0.5f * scaleFactor) : delay);
  }

  @Override
  public int attackProc(Char enemy, int damage) {
    return super.attackProc(
        enemy, (int) (damage * (Dungeon.isChallenged(Challenges.SBSG) ? (0.6f * scaleFactor) : 1)));
  }
  ...
  public int defenseSkill(Char enemy) {
    if (!surprisedBy(enemy) && paralysed == 0 && !(alignment == Alignment.ALLY && enemy == hero)) {
      return (int)
          (this.defenseSkill / (Dungeon.isChallenged(Challenges.SBSG) ? (0.4f * scaleFactor) : 1));
    } else {
      return 0;
    }
  }
  ...
  public int defenseProc(Char enemy, int damage) {
    ...
    if (buff(SoulMark.class) != null) {
      ...
      if (enemy != Dungeon.hero) {
        restoration =
            Math.round(restoration * 0.4f * Dungeon.hero.pointsInTalent(Talent.SOUL_SIPHON) / 3f);
      }
      if (restoration > 0) {
        Buff.affect(Dungeon.hero, Hunger.class)
            .affectHunger(restoration * Dungeon.hero.pointsInTalent(Talent.SOUL_EATER) / 3f);
        Dungeon.hero.HP =
            (int) Math.ceil(Math.min(Dungeon.hero.HT, Dungeon.hero.HP + (restoration * 0.4f)));
  ...
  public void die(Object cause) {
    ...
    if (!(this instanceof Wraith)
        && soulMarked
        && Random.Float()
            < (0.4f * Dungeon.hero.pointsInTalent(Talent.NECROMANCERS_MINIONS) / 3f)) {
  ...
  public void rollToDropLoot() {
    ...
    if (Dungeon.hero.buff(Talent.BountyHunterTracker.class) != null) {
      ...
      if (prep != null && Random.Float() < 0.25f * prep.attackLevel()) {
        Dungeon.level
            .drop(new Gold(15 * Dungeon.hero.pointsInTalent(Talent.BOUNTY_HUNTER)), pos)
            .sprite
            .drop();
      }
    }
    ...
    public boolean act(boolean enemyInFOV, boolean justAlerted) {
      ...
      if (enemyInFOV) {
        ...
        if (enemy instanceof Hero && ((Hero) enemy).hasTalent(Talent.SILENT_STEPS)) {
          if (Dungeon.level.distance(pos, enemy.pos)
              >= 4 - ((Hero) enemy).pointsInTalent(Talent.SILENT_STEPS)) {
    ...
    public boolean act(boolean enemyInFOV, boolean justAlerted) {
      ...
      if (enemyInFOV && !isCharmedBy(enemy) && canAttack(enemy)) {
        ...
        if (target != -1 && getCloser(target)) {
          ...
          // if moving towards an enemy isn't possible, try to switch targets to another enemy that
          // is closer
  ...
  protected class Fleeing implements AiState {
    ...
    public boolean act(boolean enemyInFOV, boolean justAlerted) {
      ...
      if (enemy == null
          || !enemyInFOV && 1 + Random.Int(Dungeon.level.distance(pos, target)) >= 6) {
        target = -1;
