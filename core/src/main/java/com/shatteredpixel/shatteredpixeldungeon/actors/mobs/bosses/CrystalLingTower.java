...
public class CrystalLingTower extends Mob {
  ...
  protected CrystalDiedTower.State countDown() {
    if (maxCount() - count > 0 && maxCount() - count < 3)
      ;
    if (count >= maxCount()) {
      count = Random.Int(0, 3) - 3;
      return CrystalDiedTower.State.SHOOTING;
  ...
  protected void zapProc() {
    ...
    if (ba.collisionPos != lastTargeting) {
      findTarget();
      Buff.affect(this, SpellCaster.HaloFireCaster.BouncePostShoot.class)
          .setTrace(ba.collisionPos, lastTargeting);
    }
    hitProc(ba);
  }
