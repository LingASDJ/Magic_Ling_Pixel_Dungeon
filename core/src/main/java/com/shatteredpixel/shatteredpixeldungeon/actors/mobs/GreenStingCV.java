...
public class GreenStingCV extends Boss {
...

    {
        ...
        if(Statistics.bossRushMode){
            initBaseStatus(25, 30, 30, 18, 480, 9, 12);
        } else {
    ...
    private boolean summonSubject( int delay ){
        ...
        if (summonsMade % 13 == 7 || summonsMade % 13 == 11) {
            ...
            return summonSubject(delay, DKGhoul.random());
        }
    ...
    private boolean summonSubject( int delay, Class<?extends Mob> type ){
        ...
        s.pos = ((GreenStlingBossLevel)Dungeon.level).getSummoningXPos();
    ...
    private HashSet<Mob> getSubjects(){
        ...
        for (Mob m : Dungeon.level.mobs){
            if (m.alignment == alignment && (m instanceof Slime || m instanceof IceGolem || m instanceof  Bat)){
                subjects.add(m);
            }
        }
        return subjects;
    ...
    public void damage(int dmg, Object src, DamageType type) {
        ...
        if (phase == 1) {
            ...
            if (HP <= 200) {
                ...
                for (Mob m : Dungeon.level.mobs.toArray(new Mob[0])) {
                    if (m instanceof Slime || m instanceof IceGolem || m instanceof  Bat) {
                        m.die(null);
                    }
                }
                for (Mob m : Dungeon.level.mobs.toArray(new Mob[0])) {
    ...
    public void die(Object cause) {
        ...
        for (Mob mob : Dungeon.level.mobs.toArray( new Mob[0] )) {
            if (	mob instanceof Shaman ||
                    mob instanceof Elemental ||
                    mob instanceof ColdMagicRat) {
                mob.die( cause );
            }
        }
        ...
        public boolean act() {
            ...
            if (delay <= 0){
                ...
                if (Actor.findChar(pos) == null) {
                    ...
                    GameScene.add(m);
                    m.state = m.HUNTING;
                    if (((GreenStingCV)target).phase == 2){
    ...
    public static class DeathRattleSummon extends Buff{
        ...
        public Mob idToMob(){
            switch (summonCate){
                case 0: return Reflection.newInstance(DKGhoul.random());
    ...
    public static class SacrificeSubjectListener extends FlavourBuff {
        ...
        public void fx(boolean on){
            if(on) {
                ...
                GLog.n(Messages.get(GreenStingCV.class,"waring"));
                //charge.on = false;
            }else{
                if(charge != null) {
                    charge.on = false;
    ...
    public static class StrengthEmpower extends FlavourBuff {
        ...
        public boolean attachTo(Char target) {
            GLog.n(Messages.get(GreenStingCV.class, "str_empower"));
