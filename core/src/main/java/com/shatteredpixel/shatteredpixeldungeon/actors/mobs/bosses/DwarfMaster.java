...
public class DwarfMaster extends Boss {
  ...
  public int attackSkill(Char target) {
    return Math.round(
        super.attackSkill(target)
            * (pumpedUp > 0 ? 3f : 1f)
            * (HP * 2 < HT ? 1.5f : 1f)
            * (surroundingWater() > 7 ? 10f : 1f));
  }

  @Override
  public int defenseSkill(Char enemy) {
    return (int)
        (super.defenseSkill(enemy)
            * ((HP * 2 <= HT) ? 1.5f : 1f)
            * (surroundingWater() > 7 ? 2f : 1f));
  }

  @Override
  public float speed() {
    return super.speed()
        * (Dungeon.level.water[pos] ? 1.33f : 1f)
        * (surroundingWater() > 7 ? 2f : 1f);
  }
  ...
  protected static boolean isSummonedByDK(Mob m) {
    return (m instanceof DwarfMaster.DKGhoul
        || m instanceof DwarfMaster.DKWarlock
        || m instanceof DwarfMaster.DKMonk);
  }
    ...
    if (abilitiesUsed >= targetAbilityUses()) {
      ...
      if (targetAbilityUses() - abilitiesUsed >= 4) {
        ...
        if (HP >= 2 + Dungeon.depth * 2) {
          ...
          int[] neighbours = {
            pos + 1, pos - 1, pos + Dungeon.level.width(), pos - Dungeon.level.width()
          };
  ...
  public static boolean throwBomb(final Char thrower, final Char target) {
    ...
    for (int i : PathFinder.NEIGHBOURS8) {
      ...
      if (Dungeon.level.distance(cell, thrower.pos) >= 3 && !Dungeon.level.solid[cell]) {
        if (targetCell == -1
            || Dungeon.level.trueDistance(cell, thrower.pos)
                < Dungeon.level.trueDistance(targetCell, thrower.pos)) {
          targetCell = cell;
        }
      }
    }
  ...
  public static class DeathRattleSummon extends Buff {
    ...
    public Mob idToMob() {
      switch (summonCate) {
        case 0:
          return Reflection.newInstance(DwarfMaster.DKGhoul.class);
        case 1:
          return Reflection.newInstance(DwarfMaster.DKWarlock.class);
        case 2:
          return Reflection.newInstance(DwarfMaster.DKMonk.class);
        case 3:
          return Reflection.newInstance(Skeleton.class);
        case 4:
          return Reflection.newInstance(SRPDHBLR.class);
        case 5:
          return Reflection.newInstance(DM201.class);
        case 6:
        default:
          return Reflection.newInstance(OGPDZSLS.class);
      }
    }
    ...
    for (Mob m : Dungeon.level.mobs.toArray(new Mob[0])) {
      if (isSummonedByDK(m)) {
        ...
        if (m.buff(DwarfMaster.DeathRattleSummon.class) == null) {
          Buff.affect(m, DwarfMaster.DeathRattleSummon.class)
              .setId(Random.chances(new float[] {9f, 4f, 4f}));
          count++;
        }
      }
      if (count >= 4) break;
  ...
  private HashSet<Mob> getSubjects() {
    ...
    for (Mob m : Dungeon.level.mobs.toArray(new Mob[0])) {
      if (m.alignment == alignment
          && (m instanceof Ghoul || m instanceof Monk || m instanceof Warlock)) {
        subjects.add(m);
      }
    }
    return subjects;
  ...
  private boolean lifeLinkSubject() {
    ...
    if (furthest != null) {
      ...
      Buff.affect(this, Healing.class).setHeal(5, 0f, 6);
      sprite.parent.add(
          new Beam.HealthRay(sprite.destinationCenter(), furthest.sprite.destinationCenter()));
      return true;
    }
    return false;
  }
  ...
  public void die(Object cause) {
    ...
    Dungeon.level
        .drop(new PotionOfHealing().quantity(Random.NormalIntRange(2, 4)), pos)
        .sprite
        .drop();
