...
public class Hero extends Char {
  ...
  public boolean isSubclass(HeroSubClass subClass) {
    if (this.subClass == HeroSubClass.ASSASSIN || this.subClass == HeroSubClass.FREERUNNER)
      return true;
  ...
  public String className() {
    ...
    return (heroSubClass == null || heroSubClass == HeroSubClass.NONE)
        ? this.heroClass.title()
        : heroSubClass.title();
  }
  ...
  public void interrupt() {
    if (isAlive()
        ...
        && ((curAction instanceof HeroAction.Move && curAction.dst != pos)
            || (curAction instanceof HeroAction.Ascend
                || curAction instanceof HeroAction.Descend))) {
  ...
  private boolean actMove(HeroAction.Move action) {
    ...
    if (chCount >= 3 && !lanterfireactive && !Dungeon.isChallenged(PRO)
        || Dungeon.isChallenged(DHXD) && !lanterfireactive) {
      // TODO 灯火前行
      lanterfire = 100;
      ...
      if (heap != null && (heap.type != Type.HEAP && heap.type != Type.FOR_SALE)) {
        ...
        if ((heap.type == Type.LOCKED_CHEST && Notes.keyCount(new GoldenKey(Dungeon.depth)) < 1)
            || (heap.type == Type.CRYSTAL_CHEST && Notes.keyCount(new CrystalKey(Dungeon.depth)) < 1
                || (heap.type == Type.BLACK && Notes.keyCount(new BlackKey(Dungeon.depth)) < 1))) {
  ...
  public int attackProc(final Char enemy, int damage) {
    ...
    switch (subClass) {
      case SNIPER:
        if (wep instanceof MissileWeapon
            && !(wep instanceof SpiritBow.SpiritArrow)
            && enemy != this) {
          Actor.add(
              new Actor() {
  ...
  public void damage(int dmg, Object src) {
    ...
    if (belongings.armor() != null
        && belongings.armor().hasGlyph(AntiMagic.class, this)
      ...
      if (fieldOfView[m.pos] && m.alignment == Alignment.ENEMY) {
        ...
        if (!mindVisionEnemies.contains(m) && QuickSlotButton.autoAim(m) != -1) {
          ...
          if (m instanceof Snake
              && Dungeon.level.distance(m.pos, pos) <= 4
  ...
  public void move(int step, boolean travelling) {
    ...
    if (Dungeon.GodWaterLevel()
        && Dungeon.level.water[pos]
        && flying
        && Dungeon.isChallenged(AQUAPHOBIA)) {
      for (Buff buff : hero.buffs()) {
        if (buff instanceof InvisibilityRing || buff instanceof HasteLing) {
          buff.detach();
        }
      }
    } else if (Dungeon.isChallenged(AQUAPHOBIA)
        && Dungeon.hero.buff(WaterSoulX.class) == null
        && Dungeon.level.water[pos]
        && Dungeon.GodWaterLevel()) {
    ...
    } else if (Dungeon.PrisonWaterLevel()
        && Dungeon.level.water[pos]
        && Dungeon.isChallenged(AQUAPHOBIA)
        && Dungeon.hero.buff(WaterSoulX.class) == null) {
    ...
    } else if (Dungeon.ColdWaterLevel()
        && Dungeon.level.water[pos]
        && Dungeon.hero.buff(WaterSoulX.class) == null
        && Dungeon.isChallenged(AQUAPHOBIA)) {
    ...
    // 矮人层
    if (Dungeon.DiedWaterLevel()
        && Dungeon.level.water[pos]
        && flying
        && Dungeon.isChallenged(AQUAPHOBIA)) {
      Buff.affect(this, HasteLing.class, Haste.DURATION / 20);
    } else if (Dungeon.DiedWaterLevel()
        && Dungeon.level.water[pos]
        && Dungeon.hero.buff(WaterSoulX.class) == null
        && Dungeon.isChallenged(AQUAPHOBIA)) {
  ...
  public boolean isImmune(Class effect) {
    ...
    if (effect == HalomethaneBurning.class
        && belongings.armor() != null
        && belongings.armor().hasGlyph(Brimstone.class, this)) {
      return true;
    }
  ...
  public boolean search(boolean intentional) {
    ...
    int left, right;
    int curr;
    for (int y = Math.max(0, c.y - distance);
        y <= Math.min(Dungeon.level.height() - 1, c.y + distance);
        y++) {
      if (!circular) {
        left = c.x - distance;
  ...
  public void resurrect() {
    ...
    for (Item i : belongings) {
      ...
      } else if (i instanceof CloakOfShadows
          && i.keptThoughLostInvent
