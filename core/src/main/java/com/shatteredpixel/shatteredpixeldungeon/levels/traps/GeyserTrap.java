...
public class GeyserTrap extends Trap {
	...
	public void activate() {
		...
		if (pos >= 0 && pos < Dungeon.level.length()) {
			...
			// 添加数组边界检查
			for (int i = 0; i < PathFinder.distance.length && i < Dungeon.level.length(); i++) {
			...
			if (ch != null){
				...
				if (centerKnockBackDirection != -1){
					...
					for (int i : PathFinder.NEIGHBOURS8){
						...
						int candidatePos = pos + i;
						int nextPos = candidatePos + i;
						if (candidatePos >= 0 && candidatePos < Dungeon.level.length() &&
								nextPos >= 0 && nextPos < Dungeon.level.length() &&
								!Dungeon.level.avoid[candidatePos] && !Dungeon.level.avoid[nextPos]){
					...
					int randomDir = PathFinder.NEIGHBOURS8[Random.Int(8)];
					int targetPos = pos + randomDir;
					if (targetPos >= 0 && targetPos < Dungeon.level.length()) {
						targetpos = targetPos;
					}
				}
