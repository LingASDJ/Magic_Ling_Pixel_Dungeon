...
public abstract class RegularLevel extends Level {
  ...
  public boolean isLevelExplored(int depth) {
    ...
    // There are no levelgen heaps which are undiscovered, in an openable container, or which
    // contain keys
    ...
    for (Blob b : blobs.values()) {
      if (b.volume > 0
          && (b instanceof MagicalFireRoom.EternalFire || b instanceof SacrificialFire)) {
        return false;
      }
    }
  ...
  protected ArrayList<Room> initRooms() {
    ...
    if (Dungeon.shopOnLevel() && !Statistics.fireGirlnoshopping) initRooms.add(new ShopRoom());

    if (Dungeon.aqiLevel() && (Dungeon.isChallenged(AQUAPHOBIA))) initRooms.add(new AquariumRoom());
    ...
    if (Dungeon.NyzshopOnLevel()) {
      Buff.affect(hero, RandomBuff.class)
          .set((3 + Random.Int(9) + hero.STR / 6 + hero.HP / 30) / Random.Int(1, 2) + 5, 1);
  ...
  protected Builder builder() {
    if (Random.Int(2) == 0) {
      return new LoopBuilder().setLoopShape(2, Random.Float(0f, 0.65f), Random.Float(0f, 0.50f));
    } else {
      return new FigureEightBuilder().setLoopShape(2, Random.Float(0.3f, 0.8f), 0f);
    }
  }
  ...
  protected void createItems() {
    ...
    // use a separate generator for this to prevent held items, meta progress, and talents from
    // affecting levelgen
    ...
    if (Dungeon.hero.hasTalent(Talent.CACHED_RATIONS)) {
      Talent.CachedRationsDropped dropped =
          Buff.affect(Dungeon.hero, Talent.CachedRationsDropped.class);
      if (dropped.count() < 2 + 2 * Dungeon.hero.pointsInTalent(Talent.CACHED_RATIONS)) {
        ...
        do {
          ...
          valid =
              cell != -1
                  && !(room(cell) instanceof SecretRoom)
