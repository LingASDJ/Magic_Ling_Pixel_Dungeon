...
public class RitualSiteRoom extends SpecialRoom {
	...
	public void paint( Level level ) {
		...
		placePlant(level, heartX + heartY * level.width(), new Elemental.NewbornFireElemental());
		...
		for (int i = 1; i <= lashers; i++){
			...
			placePlant(level, pos, new Elemental.NewbornFireElemental());
		}
	}
	...
	private static void placePlant(Level level, int pos, Mob plant){
		...
		for(int i : PathFinder.NEIGHBOURS8) {
			if (level.map[pos + i] == Terrain.EMPTY){
				Painter.set(level, pos + i, Terrain.EMPTY);
			}
		}
	}
}
