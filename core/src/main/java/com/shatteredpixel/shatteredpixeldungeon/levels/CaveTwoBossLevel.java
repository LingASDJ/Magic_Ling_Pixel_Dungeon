...
public class CaveTwoBossLevel extends Level {
    ...
    protected boolean build() {
        ...
        Painter.fillEllipse(this, mainArena, Terrain.WATER);
    ...
    public void activatePylon(){
        ArrayList<PylonCS> pylons = new ArrayList<>();
        for (Mob m : mobs){
            if (m instanceof PylonCS && m.alignment == Char.Alignment.NEUTRAL){
                pylons.add((PylonCS) m);
            }
        }
                        ...
                        for (int k : pylonPositions) {
                            if (k == j) {
                                if (Dungeon.level.locked
                                        && !(Actor.findChar(k) instanceof PylonCS)) {
                                    data[i] = 38;
                                } else {
                                    data[i] = -1;
                                }
