...
import static com.shatteredpixel.shatteredpixeldungeon.Statistics.skydeadFight;
...
public class MiniSkyShadowBossLevel extends RegularLevel {
        ...
        if (transition.type == LevelTransition.Type.BRANCH_ENTRANCE) {
            ...
            if (!skydeadFight){
                Game.runOnRenderThread(new Callback() {
                    @Override
                    public void call() {
                        GLog.w(Messages.get(SkyDead.class, "cant_enter_1"));
                    }
                });
                return false;
            }
            ...
            Game.runOnRenderThread(new Callback() {
                ...
                public void call() {
                    GameScene.show(new WndOptions( new NxhySprite(),
                            ...
                            Messages.get(SkyDead.class, "exit_warn_none"),
                            Messages.get(SkyDead.class, "exit_yes"),
                            Messages.get(SkyDead.class, "exit_no")){
                        @Override
