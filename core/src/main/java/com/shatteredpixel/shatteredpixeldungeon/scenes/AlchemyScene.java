...
public class AlchemyScene extends PixelScene {
  ...
  public void create() {
    ...
    water =
        new SkinnedBlock(Camera.main.width, Camera.main.height, Dungeon.level.waterTex()) {
    ...
    Image im =
        new Image(
            TextureCache.createGradient(
                0x66000000, 0x88000000, 0xAA000000, 0xCC000000, 0xFF000000));
    im.angle = 90;
    ...
    smokeEmitter.pos(
        outputs[0].left() + (BTN_SIZE - 16) / 2f, outputs[0].top() + (BTN_SIZE - 16) / 2f, 16, 16);
    ...
    IconButton btnGuide =
        new IconButton(new ItemSprite(ItemSpriteSheet.ALCH_PAGE, null)) {
          @Override
          protected void onClick() {
            super.onClick();
            clearSlots();
    ...
    energyLeft.setPos(
        (Camera.main.width - energyLeft.width()) / 2, Camera.main.height - 8 - energyLeft.height());
    energyLeft.hardlight(0x44CCFF);
    add(energyLeft);

    energyIcon =
        new ItemSprite(toolkit != null ? ItemSpriteSheet.ARTIFACT_TOOLKIT : ItemSpriteSheet.ENERGY);
    ...
    energyAdd =
        new IconButton(Icons.get(Icons.PLUS)) {
          @Override
          protected void onClick() {
    ...
    energyAdd.setRect(
        energyLeft.right(), energyLeft.top() - (16 - energyLeft.height()) / 2, 16, 16);
    align(energyAdd);
    add(energyAdd);
  ...
  private class CombineButton extends Component {
    ...
    protected void layout() {
      ...
      costText.setPos(left() + (width() - costText.width()) / 2, top() - costText.height());
    }
