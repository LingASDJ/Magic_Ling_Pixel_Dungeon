...
public class InterlevelScene extends PixelScene {
  ...
  private void newTipSet() {
    ...
    for (int i = 1; i <= NUM_TIPS; i++) tipset.add(i);
  }
  ...
  public void create() {
    ...
    if (tipset == null || tipset.isEmpty()) newTipSet();
    ...
    SkinnedBlock bg =
        new SkinnedBlock(Camera.main.width, Camera.main.height, loadingAsset) {
          @Override
    ...
    Image im =
        new Image(
            TextureCache.createGradient(
                0xAA000000, 0xBB000000, 0xCC000000, 0xDD000000, 0xFF000000)) {
          @Override
          public void update() {
            super.update();
    ...
    switch (phase) {
      ...
      case STATIC:
        if (error != null) {
          ...
          throw new RuntimeException(
              Messages.get(TitleScene.class, "spawnerror")
                  + "Seed:"
                  + Dungeon.seed
                  + " depth:"
                  + Dungeon.depth
