...
public class GameScene extends PixelScene {
  ...
  public void create() {
    ...
    if (uiSize == 2) {
      ...
      toolbar.setRect(
          0,
          uiCamera.height - toolbar.height() - inventory.height(),
          uiCamera.width,
          toolbar.height());
    } else {
      ...
      case DESCEND:
        switch (Dungeon.depth) {
          ...
          case 16:
            ...
            break;
            ////						case 20:
            ////							if((Statistics.boss_enhance & 0x8) != 0) WndStory.showChapter(
            // WndStory.ID_DWADA );
    ...
    if (InterlevelScene.mode != InterlevelScene.Mode.NONE) {
      ...
      if (Dungeon.hero.hasTalent(Talent.ROGUES_FORESIGHT)
        ...
        if (reqSecrets <= 0
            && Random.Int(4) <= Dungeon.hero.pointsInTalent(Talent.ROGUES_FORESIGHT)) {
  ...
  public static void layoutTags() {
    ...
    if (scene.inventory != null
        && scene.inventory.visible
        && (uiCamera.width < 460 && uiCamera.height < 300)) {
      Camera.main.setCenterOffset(
          0, Math.min(300 - uiCamera.height, 460 - uiCamera.width) / Camera.main.zoom);
    } else {
    ...
    float invWidth =
        (scene.inventory != null && scene.inventory.visible) ? scene.inventory.width() : 0;
  ...
  private void showBanner(Banner banner) {
    ...
    banner.y =
        align(uiCamera, (uiCamera.height - banner.height) / 2 - banner.height / 2 - 16 - offset);
  ...
  public static void centerNextWndOnInvPane() {
    if (scene != null && scene.inventory != null && scene.inventory.visible) {
      lastOffset =
          new Point(
              (int) scene.inventory.centerX() - uiCamera.width / 2,
  ...
  public static void gameOver() {
    ...
    StyledButton restart =
        new StyledButton(Chrome.Type.GREY_BUTTON_TR, Messages.get(StartScene.class, "new"), 9) {
          @Override
    ...
    restart.setPos(
        ...
        align(
            uiCamera, (restart.camera.height - restart.height()) / 2 + restart.height() / 2 + 16));
    scene.add(restart);

    StyledButton menu =
        new StyledButton(
            Chrome.Type.GREY_BUTTON_TR, Messages.get(WndKeyBindings.class, "menu"), 9) {
          @Override
          protected void onClick() {
    ...
    menu.setPos(align(uiCamera, (menu.camera.width - menu.width()) / 2), restart.bottom() + 10);
    scene.add(menu);
    ...
    info.setPos(align(uiCamera, (info.camera.width - info.width()) / 2), restart.bottom() - 120);
    scene.add(info);
  }
  ...
  public static void examineObject(Object o) {
    if (o == Dungeon.hero) {
      ...
      if (o instanceof Snake
          && !Document.ADVENTURERS_GUIDE.isPageRead(Document.GUIDE_SURPRISE_ATKS)) {
        GLog.p(Messages.get(Guidebook.class, "hint"));
        ...
        public void onRightClick(Integer cell) {
          ...
          if (objects.size() > 1) {
            ...
            for (int i = 1; i < textLines.size(); i++) {
              textLines.add(
                  i, "_" + Messages.get(GameScene.class, "examine") + ":_ " + textLines.remove(i));
            }
          } else {
          ...
          RightClickMenu menu =
              new RightClickMenu(image, title, textLines.toArray(new String[0])) {
                @Override
