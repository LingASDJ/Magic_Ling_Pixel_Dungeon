...
public class HeroSelectScene extends PixelScene {
  ...
  private static HeroClass[] heroClasses =
      new HeroClass[] {HeroClass.WARRIOR, HeroClass.MAGE, HeroClass.ROGUE, HeroClass.HUNTRESS};
  ...
  public void create() {
    ...
    infoButton =
        new IconButton(Icons.get(Icons.INFO)) {
          @Override
          protected void onClick() {
            super.onClick();
            if (GamesInProgress.selectedClass == null) return;
        ...
        new IconButton(
          ...
          protected void onClick() {
            if (DeviceCompat.isDebug() || Badges.isUnlocked(Badges.Badge.VICTORY)) {
              ...
              ShatteredPixelDungeon.scene()
                  .addToFront(
                      new WndMessage(Messages.get(HeroSelectScene.class, "challenges_unlock")));
          }
    ...
    IconButton prevBtn =
        new IconButton(Icons.get(Icons.LEFTBUTTON)) {
          {
            width = 20;
            height = 20;
          }
    ...
    prevBtn.setPos(
        frame.x - BUTTON_HEIGHT - FRAME_MARGIN_X, frame.y + frame.height / 2 - BUTTON_HEIGHT / 2f);
    PixelScene.align(prevBtn);
    add(prevBtn);

    IconButton nextBtn =
        new IconButton(Icons.get(Icons.RIGHTBUTTON)) {
          {
            width = 20;
            height = 20;
          }
    ...
    nextBtn.setPos(
        frame.x + frame.width + FRAME_MARGIN_X, frame.y + frame.height / 2 - BUTTON_HEIGHT / 2f);
    PixelScene.align(nextBtn);
    add(nextBtn);

    IconButton Telnetsc =
        new IconButton(new ItemSprite(ItemSpriteSheet.ARTIFACT_SPELLBOOK, null)) {
          ...
          protected void onClick() {
            ...
            a.heroClass(cl);
            ShatteredPixelDungeon.scene()
                .addToFront(new WndMessage(Messages.get(cl, cl.name() + "_story")));
          }
        };
    ...
    IconButton DevMode =
        new IconButton(Icons.get(Icons.WARNING)) {
          @Override
          protected void onClick() {
            //
          }
    ...
    DevMode.setPos(
        frame.x + frame.width + FRAME_MARGIN_X, frame.y - 14 + frame.height - 14 - BUTTON_HEIGHT);
    add(DevMode);

    IconButton Rename =
        new IconButton(Icons.get(Icons.RENAME_OFF)) {
          @Override
          protected void onClick() {
              ...
              Game.runOnRenderThread(
                  () ->
                      ShatteredPixelDungeon.scene()
                          .add(
                              new WndTextInput(
                                  Messages.get(WndStartGame.class, "custom_name"),
                                  Messages.get(WndStartGame.class, "custom_name_desc")
                                      + SPDSettings.heroName(),
                                  SPDSettings.heroName(),
                                  20,
                                  false,
                                  Messages.get(WndStartGame.class, "custom_name_set"),
    ...
    Rename.setPos(
        frame.x + frame.width + FRAME_MARGIN_X, frame.y - 38 + frame.height - 38 - BUTTON_HEIGHT);
    add(Rename);

    IconButton EverDayGo =
        new IconButton(new Image(new PinkLingSprite())) {
          @Override
          protected void onClick() {
            if (!Badges.isUnlocked(Badges.Badge.BOSS_SLAIN_1)) {
              Game.runOnRenderThread(
    ...
    EverDayGo.setPos(
        frame.x + frame.width + FRAME_MARGIN_X, frame.y - 51 + frame.height - 51 - BUTTON_HEIGHT);
    add(EverDayGo);
    ...
    if (landscape()) {
      ...
      title.setPos(
          frame.x - frame.width / 5f + FRAME_MARGIN_X / 5f,
          frame.y + frame.height / 8 - BUTTON_HEIGHT - 45);
      ...
      twotitle.setPos(
          frame.x - frame.width / 5f + FRAME_MARGIN_X / 5f,
          frame.y + frame.height / 4 - BUTTON_HEIGHT - 40);
      add(twotitle);
    }
  ...
  private void setSelectedHero() {
    ...
    startBtn.setSize(
        Math.max(startBtn.reqWidth() + 8, SKY_WIDTH - FRAME_MARGIN_X * 2), BUTTON_HEIGHT);
    startBtn.setPos(
        (Camera.main.width - startBtn.width()) / 2f, frame.y + frame.height + FRAME_MARGIN_X);
