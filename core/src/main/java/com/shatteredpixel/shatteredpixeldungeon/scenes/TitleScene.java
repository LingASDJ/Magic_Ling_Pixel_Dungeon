...
public class TitleScene extends PixelScene {
  ...
  public void create() {
    ...
    Image swordLeft =
        new Image(BannerSprites.get(BannerSprites.Type.SWORD)) {
    ...
    Image swordRight =
        new Image(BannerSprites.get(BannerSprites.Type.SWORD)) {
          ...
          public void update() {
            ...
            float time = 0.8f;
            this.x =
                Camera.main.width - curTime / time * (Camera.main.width / 2f + this.width / 2f);
            this.angle = 90 + curTime / time * 225;
            am = curTime * curTime * curTime / (time * time * time);
    ...
    StyledButton btnPlay =
        new StyledButton(GREY_TR, Messages.get(this, "enter")) {
          @Override
    ...
    StyledButton btnRankings =
        new StyledButton(GREY_TR, Messages.get(this, "rankings")) {
          @Override
          protected void onClick() {
    ...
    StyledButton btnBadges =
        new StyledButton(GREY_TR, Messages.get(this, "badges")) {
          @Override
          protected void onClick() {
    ...
    StyledButton btnAbout =
        new StyledButton(GREY_TR, Messages.get(this, "about")) {
          @Override
          protected void onClick() {
    ...
    if (landscape()) {
      ...
      align(btnPlay);
      btnRankings.setRect(
          btnPlay.left(), btnPlay.bottom() + GAP, (btnPlay.width() * 0.332f) - 1, BTN_HEIGHT);
      btnBadges.setRect(
          btnRankings.left(), btnRankings.bottom() + GAP, btnRankings.width(), BTN_HEIGHT);
      btnSupport.setRect(
          btnRankings.right() + 2, btnRankings.top(), btnRankings.width(), BTN_HEIGHT);
      btnChanges.setRect(
          btnSupport.left(), btnSupport.bottom() + GAP, btnRankings.width(), BTN_HEIGHT);
      btnSettings.setRect(
          btnSupport.right() + 2, btnSupport.top(), btnRankings.width(), BTN_HEIGHT);
      btnAbout.setRect(
          btnSettings.left(), btnSettings.bottom() + GAP, btnRankings.width(), BTN_HEIGHT);
      btnNews.setRect(
          btnPlay.left(), btnAbout.bottom() + GAP, btnAbout.width() + 157 - 1, BTN_HEIGHT);
      align(btnNews);
    } else {
      btnPlay.setRect(title.x, topRegion + GAP, title.width(), BTN_HEIGHT);
      align(btnPlay);
      btnRankings.setRect(
          btnPlay.left(), btnPlay.bottom() + GAP, (btnPlay.width() / 2) - 1, BTN_HEIGHT);
      btnBadges.setRect(
          btnRankings.right() + 2, btnRankings.top(), btnRankings.width(), BTN_HEIGHT);
      btnSupport.setRect(
          btnRankings.left(), btnRankings.bottom() + GAP, btnRankings.width(), BTN_HEIGHT);
      ...
      btnNews.setRect(
          btnPlay.left(), btnAbout.bottom() + GAP, btnAbout.width() + 68 - 1, BTN_HEIGHT);
      align(btnNews);
    }
      ...
      if (unreadCount > 0) {
        textColor(
            ColorMath.interpolate(
                0xFFFFFF, Window.TITLE_COLOR, 0.5f + (float) Math.sin(Game.timeTotal * 5) / 2f));
      }
    }
