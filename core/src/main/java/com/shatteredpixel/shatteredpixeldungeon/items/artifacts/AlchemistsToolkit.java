...
public class AlchemistsToolkit extends Artifact {
	...
	public ArrayList<String> actions( Hero hero ) {
		...
		if (isEquipped( hero ) && !cursed && !isMagicImmuned(hero)) {
			actions.add(AC_BREW);
			if (level() < levelCap) {
	...
	public void execute(Hero hero, String action ) {
		...
		if (action.equals(AC_BREW)){
			...
			if (!isEquipped(hero))              GLog.i( Messages.get(this, "need_to_equip") );
				...
				String[] options;
				if (maxLevels > 1){
					options = new String[]{ Messages.get(this, "energize_1"), Messages.get(this, "energize_all",
							5*maxLevels, maxLevels)};
				} else {
	...
	public void charge(Hero target, float amount) {
		if (isMagicImmuned(target)) return;
		...
		public boolean act() {
			...
			if (warmUpDelay > 0){
				if (level() == 10){
					...
					warmUpDelay = 100f;
				} else if (!cursed && !isMagicImmuned(target)) {
