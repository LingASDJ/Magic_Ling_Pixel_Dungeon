...
public class WandOfBlueFuck extends DamageWand {
  ...
  public void onZap(Ballistica bolt) {
    ...
    // ignite cells that share a side with an adjacent cell, are flammable, and are further from the
    // source pos
    // This prevents short-range casts not igniting barricades or bookshelves
    for (int cell : adjacentCells) {
      for (int i : PathFinder.CIRCLE4) {
        if (Dungeon.level.trueDistance(cell + i, bolt.sourcePos)
                > Dungeon.level.trueDistance(cell, bolt.sourcePos)
  ...
  public void fx(Ballistica bolt, Callback callback) {
    ...
    for (Ballistica ray : cone.rays) {
      ((MagicMissile) curUser.sprite.parent.recycle(MagicMissile.class))
          .reset(MagicMissile.EARTH, curUser.sprite, ray.path.get(ray.dist), null);
    }
  ...
  public String statsDesc() {
    if (levelKnown) return Messages.get(this, "stats_desc", chargesPerCast(), min(), max());
    else return Messages.get(this, "stats_desc", chargesPerCast(), min(0), max(0));
  }
