...
public class BrokenRingArtifactBind extends Artifact {

    public Artifact bound;   // 被束缚的真神器（本体存在这里）

    // 束缚真神器时调用：先存本体引用，再抄图标
    public void mimic(Artifact original) {
        bound = original;
    ...
    public void restoreFromBundle(Bundle bundle) {
        ...
        bound = (Artifact) bundle.get("bound");
        if (bound != null) {
            image = bound.image;
        }
    }
