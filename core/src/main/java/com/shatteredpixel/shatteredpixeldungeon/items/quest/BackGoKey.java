  ...
  public static class WndSelectLevel extends Window {
    ...
    public WndSelectLevel() {
      ...
      for (int i = 0; i < Constants.MAX_DEPTH; ++i) {
        ...
        int row = i / 5;
        final int j = i + 1;
        LevelTeleporter.DepthButton db =
            new LevelTeleporter.DepthButton(j) {
              @Override
      ...
      sp.setRect(
          ...
          ttl.bottom() + GAP * 2,
          WIDTH,
          Math.min(btns.get(btns.size() - 1).bottom(), PANE_MAX_HEIGHT));

      icb =
          new StyledButton(
              Chrome.Type.RED_BUTTON,
              M.L(LevelTeleporter.class, "interlevel_teleport_go", selectedLevel)) {
            @Override
            protected void onClick() {
              super.onClick();
