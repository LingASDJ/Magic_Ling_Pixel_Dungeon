...
public class RedBloodMoon extends MeleeWeapon {
  ...
  public int damageRoll(Char var1) {
    if (var1 instanceof Hero) {
      ...
      if (var3 instanceof Mob && ((Mob) var3).surprisedBy(var2)) {
        ...
        int var5 = this.min();
        var4 =
            this.augment.damageFactor(
                Random.NormalIntRange(
                    this.min() + Math.round((float) (var4 - var5) * 0.9F), this.max()));
  ...
  public int min(int var1) {
    return Math.round((float) (this.tier + 0) * 1.08F) * var1
        + Math.round((float) (this.tier + 1) * 0.6675F);
  }
  ...
  public static class SRPDHBLRTT
      extends com.shatteredpixel.shatteredpixeldungeon.sprites.SRPDHBLRTT {
