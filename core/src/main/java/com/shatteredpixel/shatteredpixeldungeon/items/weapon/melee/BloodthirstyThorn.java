...
public class BloodthirstyThorn extends MeleeWeapon {
	...
	protected void duelistAbility(Hero hero, Integer target) {
		...
		int dmgBoost = augment.damageFactor(8 + Math.round(1.5f*buffedLvl()));
		Mace.heavyBlowAbility(hero, target, 1, dmgBoost, this);
	}

	@Override
	public String abilityInfo() {
		int dmgBoost = levelKnown ? 8 + Math.round(1.5f*buffedLvl()) : 8;
		if (levelKnown){
	...
	public String upgradeAbilityStat(int level){
		int dmgBoost = 8 + Math.round(1.5f*level);
