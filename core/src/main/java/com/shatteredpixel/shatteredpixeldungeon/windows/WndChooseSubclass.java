...
public class WndChooseSubclass extends Window {
	...
	public static class WndOldMageClass extends Window {

		private static final int WIDTH		= 130;
		private static final float GAP		= 2;

		public WndOldMageClass(final OldSunShadow tome, final Hero hero ) {
			...
			for (HeroSubClass subCls : hero.heroClass.subClasses()){
				...
				RedButton btnCls = new RedButton( subCls.shortDesc(), 6 ) {
					...
					protected void onClick() {
						GameScene.show(new WndOptions(new HeroIcon(subCls),
								Messages.titleCase(subCls.title()),
								Messages.get( WndOldMageClass.class, "are_you_sure"),
								Messages.get( WndOldMageClass.class, "yes"),
								Messages.get( WndOldMageClass.class, "no")){
							@Override
							protected void onSelect(int index) {
								hide();
								if (index == 0 &&  WndOldMageClass.this.parent != null){
									 WndOldMageClass.this.hide();
									tome.choose( subCls );
								}
							}
							@Override
